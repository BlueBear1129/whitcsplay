...
class Lessonstwo extends StatelessWidget {
  const Lessonstwo({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const LessonstwoPage(title: 'Lessons Page'),
    );
  }
}

class LessonstwoPage extends StatefulWidget {
  const LessonstwoPage({Key? key, required this.title}) : super(key: key);

  final String title;

  @override
  State<LessonstwoPage> createState() => _LessonstwoPageState();
}

class _LessonstwoPageState extends State<LessonstwoPage> {
  // displaying text on the green part of the chalk board from a txt file.
  String? datatwo;
  
  void _loadData() async {
    final _loadedData =
        await rootBundle.loadString('lib/textfile/lessonsvariables.txt');
    setState(() {
      datatwo = _loadedData;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Variables Lessons Page'),
        leading: IconButton(
      ...
      body: Container(
        ...
        child: FittedBox(
          ...
          child: Stack(
            children: <Widget>[
              const Text(' Variables Lessons Page'),
              ...
              Positioned(
                top: 25,
                left: 18,
                child: TextBox(text: datatwo ?? 'empty'),
              ),
              ...
              Positioned(
                     top: 60,
                     left: 30,
                     child: Text(datatwo ?? 'empty',style: const TextStyle(color: Colors.white))
